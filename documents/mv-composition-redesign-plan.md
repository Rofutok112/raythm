...
- ユーザー向けの第一級概念は Object / Component / Transform / Keyframe / Asset とする。
...
New song-scoped MV compositions initialize root `durationMs` and default full-song layers from the parent song duration. If the song duration is unavailable, the editor uses a long authoring fallback instead of an 8-second preview-sized clip. Layer `startMs`, layer `durationMs`, and keyframe `timeMs` are all song-timeline milliseconds.
...
- Effect IDs must be non-empty and unique within their layer.
...
Song/MV event triggers are intentionally out of scope for the current editor model.
The immediate authoring path is Object timing, Components, and Keyframes. Event-like
behavior should be reconsidered later only if normal timeline/keyframe editing cannot
cover the workflow.

Rules:

- Do not add `EventTrigger` or Cue authoring until the Timeline and Animation lanes are stable.
- Do not store event actions in `composition.rmvcomp` for now.
- Prefer explicit Object spans and Keyframes over hidden timeline triggers.
...
- Source, Transform, Effect, Keyframe, and Asset references are represented in authoring data.
...
- Do not add built-in presets until the Object/Component/Timeline authoring model is stable.
- Do not reintroduce free-form direct drawing scripts for standard expressions.

Current implementation status:

- Effect components round-trip in `composition.rmvcomp`.
...
- Built-in preset code has been removed for now so the editor stays focused on explicit Object/Component authoring.
- Effect ordering UI, per-effect selection, keyframed effect parameters, and audio-reactive effects are still pending.

Done when:

- Common motion graphics effects can be authored without script hooks.
- Effect parameter changes can be keyframed.
- Effects remain ordinary components with no separate preset runtime path.
...
## Stage 11: Song Visual Events

Song visual events are deferred. The previous EventTrigger/Cue prototype has been
removed because it was not yet used and made the Timeline model harder to understand.

Future reconsideration rules:

- Revisit this only after Object spans, Keyframes, and Animation lanes are production-usable.
- Any future event system must not appear as hidden Cue entries in the basic Timeline menu.
- Event data should be added only when there is a concrete song-synchronized workflow that keyframes cannot express cleanly.

Done when:

- A real song-event workflow exists and has a UI that is distinct from ordinary keyframe editing.
...
- reusable authoring templates, if the editor model later needs them
...
4. Stage 8 to Stage 10: add keyframes, standard effects, generated sources, and audio reactivity.
5. Stage 11: defer song visual events until the keyframe/timeline model is stable.
...
- `mv_composition_model_smoke` for model defaults, JSON round-trip, fingerprint stability, assets, and unsupported version rejection.
...
- Reconsider song visual events after timeline/keyframe editing is stable.
