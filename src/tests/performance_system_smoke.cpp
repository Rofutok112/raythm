...
int main() {
    ...
    if (performance.max_pp() <= 0.0f) {
        std::cerr << "Expected chart max pp to be positive\n";
        return EXIT_FAILURE;
    }

    performance.on_judge(perfect_event(0));
    const float first_delta = performance.current_pp();
    performance.on_judge(perfect_event(1));
    performance.on_judge(perfect_event(2));
    const float before_burst = performance.current_pp();
    performance.on_judge(perfect_event(3));
    const float burst_delta = performance.current_pp() - before_burst;

    if (first_delta <= 0.0f || burst_delta <= first_delta * 1.5f) {
        std::cerr << "Expected burst event to award substantially more live pp than an easy opener: "
    ...
    const float after_miss = performance.current_pp();
    performance.on_judge(perfect_event(5));
    if (performance.current_pp() <= after_miss) {
        std::cerr << "Expected later clean hits to keep increasing live pp after a miss\n";
        return EXIT_FAILURE;
    }
