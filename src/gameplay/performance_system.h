...
public:
    ...
    [[nodiscard]] float current_rc() const;
    [[nodiscard]] float max_rc() const;
    [[nodiscard]] float weighted_accuracy() const;

private:
    [[nodiscard]] float calculate_current_rc() const;
    ...
    float chart_max_rc_ = 0.0f;
    int judged_events_ = 0;
    int combo_ = 0;
    int max_combo_ = 0;
    std::array<int, 5> judge_counts_ = {};
    float cached_current_rc_ = 0.0f;
};
