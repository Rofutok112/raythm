...
std::vector<size_t> judge_system::find_press_candidates(int lane, double timestamp_ms) {
    ...
    std::optional<double> hold_head_target_time_ms;
    double best_hold_head_abs_offset = kBadWindowMs + 1.0;
    for (size_t descriptor_index = 0; descriptor_index < event_descriptors_.size(); ++descriptor_index) {
        ...
        const double abs_offset = std::fabs(offset_ms);
        if (abs_offset < best_hold_head_abs_offset) {
            hold_head_target_time_ms = descriptor.time_ms;
            best_hold_head_abs_offset = abs_offset;
        }
    }

    if (hold_head_target_time_ms.has_value()) {
        for (size_t descriptor_index = 0; descriptor_index < event_descriptors_.size(); ++descriptor_index) {
            const chart_judge_event& descriptor = event_descriptors_[descriptor_index];
            if (event_completed_[descriptor_index]) {
                continue;
            ...
            if (!same_judgement_time(descriptor.time_ms, *hold_head_target_time_ms)) {
                continue;
            }
