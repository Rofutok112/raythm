...
const song_entry* selected_song(const state& state) {
    if (state.catalog.songs.empty() || state.selection.song_index < 0 ||
        state.selection.song_index >= static_cast<int>(state.catalog.songs.size())) {
        return nullptr;
    }

    return &state.catalog.songs[static_cast<size_t>(state.selection.song_index)];
}
...
const chart_option* selected_chart_for(const state& state, const std::vector<const chart_option*>& filtered) {
    ...
    const int index = std::min<int>(state.selection.chart_index, static_cast<int>(filtered.size()) - 1);
    return filtered[static_cast<size_t>(index)];
}

void reset_for_enter(state& state) {
    state.catalog.loading = false;
    state.catalog.loaded_once = false;
    state.selection.song_index = 0;
    state.selection.chart_index = 0;
    state.song_list_scroll.y = 0.0f;
    state.song_list_scroll.target_y = 0.0f;
    state.chart_list_scroll.y = 0.0f;
    state.chart_list_scroll.target_y = 0.0f;
    state.preview.song_change_anim_t = 1.0f;
    state.preview.chart_change_anim_t = 1.0f;
    state.transition.fade_in.restart(scene_fade::direction::in, 0.3f, 0.65f);
    state.song_list_scroll.scrollbar_dragging = false;
    state.song_list_scroll.scrollbar_drag_offset = 0.0f;
    state.context_menu = {};
...
void tick_animations(state& state, float dt) {
    state.preview.song_change_anim_t = tween::retreat(state.preview.song_change_anim_t, dt, 4.0f);
    state.preview.chart_change_anim_t = tween::retreat(state.preview.chart_change_anim_t, dt, 5.0f);
    ...
    state.transition.fade_in.update(dt);
}

void apply_catalog(state& state, catalog_data catalog,
                   const std::string& preferred_song_id,
                   const std::string& preferred_chart_id) {
    state.catalog.songs = std::move(catalog.songs);
    state.catalog.load_errors = std::move(catalog.load_errors);
    state.catalog.loading = false;
    state.catalog.loaded_once = true;
    state.selection.song_index = 0;
    state.selection.chart_index = 0;
    state.song_list_scroll.y = 0.0f;
    state.song_list_scroll.target_y = 0.0f;
    ...
    if (!preferred_song_id.empty()) {
        for (int i = 0; i < static_cast<int>(state.catalog.songs.size()); ++i) {
            if (state.catalog.songs[static_cast<size_t>(i)].song.meta.song_id == preferred_song_id) {
                state.selection.song_index = i;
                break;
            }
        }
    }

    if (!state.catalog.songs.empty() && !preferred_chart_id.empty()) {
        const auto& charts = state.catalog.songs[static_cast<size_t>(state.selection.song_index)].charts;
        for (int i = 0; i < static_cast<int>(charts.size()); ++i) {
            if (charts[static_cast<size_t>(i)].meta.chart_id == preferred_chart_id) {
                state.selection.chart_index = i;
                break;
            }
        }
    }

    if (!state.catalog.songs.empty()) {
        state.preview.song_change_anim_t = 1.0f;
        state.preview.chart_change_anim_t = 1.0f;
        const float restored_scroll = scroll_offset_for_selected_song(state);
        state.song_list_scroll.y = restored_scroll;
        state.song_list_scroll.target_y = restored_scroll;
    }
}
...
bool apply_song_selection(state& state, int song_index, int chart_index) {
    ...
    const int clamped_song_index = std::clamp(song_index, 0, static_cast<int>(state.catalog.songs.size()) - 1);
    const bool song_changed = clamped_song_index != state.selection.song_index;
    const int previous_chart_index = state.selection.chart_index;
    state.selection.song_index = clamped_song_index;

    const auto filtered = filtered_charts_for_selected_song(state);
    if (filtered.empty()) {
        state.selection.chart_index = 0;
    } else {
        state.selection.chart_index = std::clamp(chart_index, 0, static_cast<int>(filtered.size()) - 1);
    }
    ...
    if (song_changed || previous_chart_index != state.selection.chart_index) {
        state.preview.chart_change_anim_t = 1.0f;
    }
    return song_changed;
}
...
void open_list_background_context_menu(state& state, Rectangle rect) {
    ...
    state.context_menu.chart_index = state.selection.chart_index;
    state.context_menu.rect = rect;
}
...
float expanded_row_height(const state& state, int song_index) {
    if (song_index == state.selection.song_index) {
...
float song_list_first_item_y(const state& state) {
    return layout::kSongListViewRect.y + song_list_content_top() - state.song_list_scroll.y;
}
...
float scroll_offset_for_selected_song(const state& state) {
    ...
    for (int i = 0; i < state.selection.song_index; ++i) {
        row_top += expanded_row_height(state, i);
    }

    const float row_bottom = row_top + expanded_row_height(state, state.selection.song_index);
...
std::string fallback_song_id_after_song_delete(const state& state, int song_index) {
    if (state.catalog.songs.size() <= 1) {
        return "";
    }
    if (song_index + 1 < static_cast<int>(state.catalog.songs.size())) {
        return state.catalog.songs[static_cast<size_t>(song_index + 1)].song.meta.song_id;
    }
    if (song_index > 0) {
        return state.catalog.songs[static_cast<size_t>(song_index - 1)].song.meta.song_id;
    }
