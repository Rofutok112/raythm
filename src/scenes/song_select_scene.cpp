...
void song_select_scene::reload_song_library(const std::string& preferred_song_id,
    ...
    data_controller_.request_catalog_reload(
        state_,
        song_select::catalog_reload_request{preferred_song_id_, preferred_chart_id_, false});
}

void song_select_scene::poll_song_library_reload() {
    const song_select::catalog_reload_result result = data_controller_.poll_catalog_reload(state_);
    if (result.completed) {
